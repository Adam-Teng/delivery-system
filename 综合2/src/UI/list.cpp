	...
	while (loop)
	{
		...
		for (auto& v : m)
		{
			...
			if (d->sendUser.find(temp) == string::npos ||
				(search.length() > 0 &&
					d->receiveUser.find(search) == string::npos &&
					d->receiveTime.find(search) == string::npos &&
					d->sendTime.find(search) == string::npos &&
					d->id.find(search) == string::npos
					))
...
void CLI::listSendCourierExpress(User* user)
{
	bool loop = true;
	string temp = user->username;
	string search = "";
...
void CLI::listHaveSCourierExpress(User* user)
{
	bool loop = true;
	string temp = user->username;
	string search = "";
