...
typedef enum DATATYPE { UNDEFINED, STAND, ONLINE } DATATYPE;
...
private:
	...
	static void destroy(Data* p) { delete p; }
...
public:
	...
	void* conn;
	static shared_ptr<Data> getInstance() { return instance; }
	/*初始化为独立模式或服务端*/
	bool init(string userFile, string deliveryFile) {
		...
		this->delivery =
			(DeliveryManager*)new Backend::DeliveryManager(deliveryFile);
		return true;
	}
};
